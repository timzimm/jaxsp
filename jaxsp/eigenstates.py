...
from jaxopt import Broyden, LBFGSB, ProjectedGradient, Bisection
...
def minimum_of_effective_potential(r_ta, l, potential_params):
    pg = LBFGSB(
        fun=lambda logr: jnp.log(
            V_effective(jnp.exp(logr), l, potential_params)
            - potential(0.0, potential_params),
        ),
        tol=1e-8,
    )
...
@jax.jit
def lower_turning_point(upper_r_ta, l, potential_params):
    pg = ProjectedGradient(
        fun=lambda logr: jnp.log(
            (V_effective(jnp.exp(logr), l, potential_params) - E) ** 2
        ),
...
@jax.jit
def sommerfeld_estimate_of_nmax(r_ta, potential_params):
    def p_effective(r, E):
...
@jax.jit
def wkb_estimate_of_rmin(r_ta, l, potential_params):
    def wkb_condition_Veff(r_lower, r_upper, Emax):
        return (
            jnp.sqrt(2)
            * quad(
...
@jax.jit
def lmax(r_ta, potential_params):
...
def init_eigenstate_library_cheb(potential_params, r_ta, N):
    @jax.jit
    @jax.vmap
...
def init_eigenstate_library_fd(potential_params, r_ta, N):
    result_shape = jax.ShapeDtypeStruct((), jnp.int64)
    name = jax.pure_callback(
        eigenstate_library.compute_name, result_shape, potential_params, r_ta, N
    )
...
def init_eigenstate_library_fd_loglin(potential_params, r_ta, N):
    result_shape = jax.ShapeDtypeStruct((), jnp.int64)
    name = jax.pure_callback(
        eigenstate_library.compute_name, result_shape, potential_params, r_ta, N
    )
